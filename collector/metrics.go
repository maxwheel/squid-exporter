...
func New(hostname string, port int, login string, password string, labels config.Labels, withProxyProtocol bool) *Exporter {
	counters = generateSquidCounters(labels.Keys)
	if ExtractServiceTimes {
		serviceTimes = generateSquidServiceTimes(labels.Keys)
	}
	c := NewCacheObjectClient(hostname, port, login, password, withProxyProtocol)
